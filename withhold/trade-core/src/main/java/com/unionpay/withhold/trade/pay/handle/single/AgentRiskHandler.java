...
@Component("agentRiskHandler")
public class AgentRiskHandler implements EventHandler<TradeBean>{
	...
	public void onEvent(TradeBean tradeBean, long sequence, boolean endOfBatch) throws Exception {
		...
		try {
			...
			riskBean.setCaCode(tradeBean.getTxnLogPayDO().getAccfirmerno());
			resultBean = tradeRiskService.agentRiskControl(riskBean);
		} catch (Exception e) {
			e.printStackTrace();
			resultBean = new ResultBean("PC037", "系统异常，风控系统异常");
			resultBean.setResultBool(false);
		} finally {
			tradeBean.setAgentRisk(resultBean);
		}
	}
