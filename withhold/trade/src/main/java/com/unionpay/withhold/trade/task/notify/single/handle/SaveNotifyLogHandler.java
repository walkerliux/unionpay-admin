...
@Component("saveNotifyLogHandler")
public class SaveNotifyLogHandler implements EventHandler<TradeNotifyBean>{
	...
	public void onEvent(TradeNotifyBean tradeNotifyBean, long sequence, boolean endOfBatch) throws Exception {
		for(ResultBean resultBean : tradeNotifyBean.getNotifyResultList()) {
			...
			}else if (resultBean.getSecondObj() instanceof OrderCollectSingleTaskDO) {
				OrderCollectSingleTaskDO orderCollectSingle = (OrderCollectSingleTaskDO) resultBean.getSecondObj();
				TxnsNotifyTaskDO txnsNotifyTask = new TxnsNotifyTaskDO();
				txnsNotifyTask.setMemberId(orderCollectSingle.getMerid());
			    txnsNotifyTask.setTxnseqno(orderCollectSingle.getRelatetradetxn());
