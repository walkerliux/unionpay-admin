...
<script>
	...
	function showAdd() {
		...
		output += 		'交易笔数累计分段扣率<input type="hidden" id="tid" name="tid"/>';
		output += 	'</td>';
		output += '</tr>';
		...
		for (var i = 1; i <= segmentIndex; i++) {
			...
			if (i > 1) {
				...
				if(preRange >= thisRange){
					$.messager.alert("","段" + i + "的分界线笔数要大于段" + (i-1) + "的分界线笔数","warning");
					return false;
				}
			}
		}
	...
	function showModify(rateId, rateDesc, accMode, notes) {
		...
		$.ajax({
			...
			success : function(json) {
				...
				output += 	'<td class="update" width="15%">扣率描述</td>';
				output += 	'<td class="update" width="30%">';
				output += 		'<input id="rateDes_a" name="rateDes" maxlength="64" class="easyui-validatebox" required="true" missingMessage="请输入扣率描述" value="' + rateDesc + '"/>';
				output += 	'</td>';
				output += 	'<td class="update" width="15%">累计方式</td>';
				output += 	'<td class="update">';
				output += 		'<select id="accMode_a" name="accMode" class="easyui-validatebox" required="true" missingMessage="请选择累计方式">';
				...
				for (var i = 0; i < rows.length; i++) {
					...
					if (i != 0) {
						...
						output += 	'</td>';
						output += 	'<td class="update" colspan="3">';
						output += 		'<input id="startRange_a' + (i + 1) + '" name="ardList[' + i + '].startRange" value="' + rows[i].STARTRANGE + '" required="true" validType="amount" maxlength="12" type="text" class="easyui-validatebox" missingMessage="请输入分界线笔数" />';
						output +=	'</td>';
						output += '</tr>';
					}
					...
					output += 			'<option value="1" ' + (rows[i].RATE_TYPE == 1 ? 'selected="selected"' : '') + '>固定金额</option>';
					output += 			'<option value="2" ' + (rows[i].RATE_TYPE == 2 ? 'selected="selected"' : '') + '>固定比例</option>';
					output += 			'<option value="3" ' + (rows[i].RATE_TYPE == 3 ? 'selected="selected"' : '') + '>固定比例+限额</option>';
					output += 		'</select>'
					output += 	'</td>';
					if (rows[i].RATE_TYPE == 1) {
						// 添加固定金额要显示的部
						output += '<td class="update rateChanged' + (i + 1) + '" width="15%">收费额(元)</td>';
						output += '<td class="update rateChanged' + (i + 1) + '" width="30%">';
						output += 	'<input id="minFee_a' + (i + 1) + '" name="ardList[' + i + '].minFee" maxlength="12" value="' + rows[i].MIN_FEE + '" required="true" validType="amount" type="text" class="easyui-validatebox" missingMessage="请输入收费金额" />';
						output += '</td>';
					}else if (rows[i].RATE_TYPE >1) {
						// 添加 固定比例 或 固定比例+限额  要显示的费率部分
						output += '<td class="update rateChanged' + (i + 1) + '" width="15%">费率（百分比）</td>';
						output += '<td class="update rateChanged' + (i + 1) + '" width="30%">';
						output += 	'<input id="feeRate_a' + (i + 1) + '" name="ardList[' + i + '].feeRate" validType="percent" value="' + rows[i].FEE_RATE + '" required="true" type="text" class="easyui-validatebox" maxlength="4" missingMessage="请输入费率" />';
						output += '</td>';
					}
					output += '</tr>'
					// 添加固定比例+限额要显示的收费额部分
					if (rows[i].RATE_TYPE == 3) {
						output += '<tr class="rateChanged' + (i + 1) + ' seg' + (i + 1) + '">';
						output += 	'<td class="update" width="15%">最低收费额(元)</td>';
						output += 	'<td class="update" width="30%">';
						output += 		'<input id="minFee_a' + (i + 1) + '" name="ardList[' + i + '].minFee" maxlength="12" value="' + rows[i].MIN_FEE + '" required="true" validType="amount" type="text" class="easyui-validatebox" missingMessage="请输入最低收费金额" />';
						output += 	'</td>';
						output += 	'<td class="update" width="15%">最高收费额(元)</td>';
						output += 	'<td class="update">';
						output += 		'<input id="maxFee_a' + (i + 1) + '" name="ardList[' + i + '].maxFee" maxlength="12" value="' + rows[i].MAX_FEE + '" required="true" validType="amount" type="text" class="easyui-validatebox" missingMessage="请输入最高收费金额" />';
						output += 	'</td>';
						output += '</tr>';
					}
				} 
	...
	function showDetail(rateId, rateDesc, accMode, notes) {
		...
		$.ajax({
			...
			dataType : "json",
			success : function(json) {
				output += '<tr><td colspan="4" class="head-title update">交易笔数累计分段扣率</td></tr>';
				...
				for (var i = 1; i <= rows.length; i++) {
					var row = rows[i - 1];
					if (row.RATE_TYPE == 1) {// 固定金额
						output += '<tr><td colspan="4" class="head-title update">段' + i +'</td></tr>';
						heightAdd += 26;
						if (i != 1) {
							output += '<tr><td class="update" width="15%">分界线' + (i-1) + '(笔数)</td><td class="update" colspan="3">' + row.STARTRANGE + '</td></tr>';
							heightAdd += 26;
						}
						output += '<tr><td class="update" width="15%">计费方式</td><td class="update" width="30%">固定金额</td>';
						output += '<td class="update" width="15%">收费额(元)</td><td class="update">' + row.MIN_FEE + '</td></tr>';
						heightAdd += 26;
					} else if (row.RATE_TYPE == 2) {// 固定比例
						output += '<tr><td colspan="4" class="head-title update">段' + i +'</td></tr>';
						heightAdd += 26;
						if (i != 1) {
							output += '<tr><td class="update" width="15%">分界线' + (i-1) + '(笔数)</td><td class="update" colspan="3">' + row.STARTRANGE + '</td></tr>';
						...
						output += '<td class="update" width="15%">费率（百分比）</td><td class="update">' + row.FEE_RATE + '</td></tr>';
						heightAdd += 26;
					}else if (row.RATE_TYPE == 3) {// 固定比例+限额
						output += '<tr><td colspan="4" class="head-title update">段' + i +'</td></tr>';
						heightAdd += 26;
						if (i != 1) {
							output += '<tr><td class="update" width="15%">分界线' + (i-1) + '(笔数)</td><td class="update" colspan="3">' + row.STARTRANGE + '</td></tr>';
						...
						output += '<td class="update" width="15%">费率（百分比）</td><td class="update">' + row.FEE_RATE + '</td></tr>';
						heightAdd += 26;
						output += '<tr><td class="update" width="15%">最低收费额(元)</td><td class="update" width="30%">' + row.MIN_FEE + '</td>';
						output += '<td class="update" width="15%">最高收费额(元)</td><td class="update">' + row.MAX_FEE + '</td></tr>';
						heightAdd += 26;
					}
				}
	...
	function addSegment(){
		...
		output += 	'<td class="update" width="15%">';
		output += 		'分界线' + segmentIndex + '(笔数)';
		output += 	'</td>';
		output += 	'<td class="update" colspan="3">';
		output += 		'<input id="startRange_a' + (segmentIndex + 1) + '" name="ardList[' + segmentIndex + '].startRange" required="true" validType="amount" maxlength="12" type="text" class="easyui-validatebox" missingMessage="请输入分界线笔数" />';
		output +=	'</td>';
		output += '</tr>';
		// 拼接计费方式
