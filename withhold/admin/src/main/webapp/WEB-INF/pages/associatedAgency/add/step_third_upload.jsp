	...
	<div style="margin: 5px;" id="continer">
		<div id="title" name="title" class="easyui-panel" title="委托机构开通"
			...
			<div style="padding-left: 5px; padding-right: 5px">
				<form id="merchDateForm" action="associatedagency/commitMerch" method="post" enctype="multipart/form-data">
					<input type="hidden" id="merchApplyId" value="${merchApplyId}" />
					<input type="hidden" id="merchId" name="merchId" value="${merchDeta.merchId}" /> 
					<input type="hidden" id="memberId" value="${merchDeta.memberId}" /> 
					...
					<table width="100%">
						<tr>
							<td colspan="4" class="head-guide">
								第一步:委托机构信息录入<img src="<%=basePath%>pictuer/tag.jpg" style="height: 10px"/>第二步:配置计费方式<img src="<%=basePath%>pictuer/tag.jpg" style="height: 10px"/><font class="current-step">第三步:上传证件照片</font>
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td><input style="height: 30px" id="orgCert_cert_img" type="file" name="headImage" class="easyui-validatebox">
...
<script>
	$(function(){
	  $("input[id*='_cert_img']").each(function(){
		  ...
		  $(this).uploadify({
				...
				'uploader': '<%=basePath%>associatedagency/upload',
				'fileObjName': 'headImage',
				'formData': {'merchApplyId':$('#merchApplyId').val(),'memberId':$('#memberId').val(),'certTypeCode':certType},
				'method' : 'post',
				//可选  
				'height': 20,
				//可选  
	...
	function toFirstStep(){
		var flag = 1;
		window.location.href= "<%=basePath%>" +'associatedagency/showAgencyAddSecond?merchApplyId=' + $('#merchApplyId').val() + '&flag=' + flag;
	}
		...
		$("input[id*='_old']").each(function(){
			...
			$.ajax({
				...
				success: function(json) {
					 if(json.status=='OK'){
						 var URL = json.url;
						 certSpan.html('<a target="view_window" style="font-size: 12px;color:blue" onclick="showDownLoadFile('+'\''+URL+'\''+')">点击查看</a>');
	...
	function showDownLoadFile(value){
		$.ajax({
		   ...
		   data: {"filePath":value,"merchApplyId":$('#memberId').val()},
		   async: true,
		   dataType:"json",
