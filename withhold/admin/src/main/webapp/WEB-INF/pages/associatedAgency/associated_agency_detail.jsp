...
<body>
	<div style="margin: 5px;" id="continer">
		<div id="title" name="title" class="easyui-panel" title="委托机构信息" style="background: #fafafa;height: 950px;" iconCls="icon-save" data-options="fit:true,border:false" collapsible="false">
...
<%-- 						</c:if> --%>
						...
						<tr>
							<td colspan="4" class="head-title">七、审核意见</td>
						</tr>
						<c:if test="${flag==2}">
							<tr>
	...
	<div region="south" border="false" style="text-align: center; padding: 5px 0;">
		<c:if test="${flag==2}">
			<a href="javascript:showUser();" id="button_ins1"
...
<script>
  ...
  $(function() {
	...
	function outerHeight(type){
		if(type == "1"){
			if(flag==2){
				$("#title").css("height",1130);
			}else if(flag==3){
				$("#title").css("height",1230);
			}else if(flag==10){
				$("#title").css("height",1180);
			}
		}else if(type == "2"){
			if(flag==2){
				$("#title").css("height",1380);
			}else if(flag==3){
				$("#title").css("height",1450);
			}else if(flag==10){
...
// 						 _this.html('<a href="javascript:showDownLoadFile();" target="view_window" style="font-size: 12px;color:blue">点击查看</a>');
						...
						$.each(json,function(key, value) {
							...
							if (value.INFO == "操作成功!") {
								$.messager.alert('提示', '操作成功!','info',function(){
									if(flag == 2){
										window.location.href="<%=basePath%>" +'associatedagency/showMerchAuditQuery';
									}else if(flag == 3){
										window.location.href="<%=basePath%>" +'associatedagency/showMerchReAuditQuery';
									}
								});
							}else{
