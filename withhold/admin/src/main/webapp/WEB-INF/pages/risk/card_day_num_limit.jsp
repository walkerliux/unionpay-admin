		...
		var panelVertFloat = 150;
		 ...
		 $(function(){
			...
			$('#test').datagrid({
				...
				nowrap: false,
				striped: true,
				url:'riskLimitCardNum/queryLimitMemNumsDay',
				remoteSort: false,
				columns:[
				[
...
//			$('#Notes').val('');
	...
	function showLimitMem(tid){		
		$.ajax({
		   type: "POST",
		   url: "riskLimitCardNum/queryLimitMumDaybytid",
		   data: "tid="+tid,
		   dataType:"json",
		   success: function(json){
		...
		$("#theForm").attr("action","riskLimitCardNum/updateLimitMemMNumDay");
		$('#btn_submit').linkbutton('enable'); 
	}
	function deleteLimitMem(tid){
		$.messager.confirm('提示','您是否想要注销此日商户累计限次?',function(r){   
		    if (r){  
		    	$.ajax({
					type: "GET",
				  	url: "riskLimitCardNum/deleteLimitMemNumDay",
				  	data: "&tid="+tid,
				 	dataType: "text",
	...
	function startLimitMem(tid){
		$.messager.confirm('提示','您是否想要启用此商户日累计限次?',function(r){   
		    if (r){  
		    	$.ajax({
					type: "GET",
				  	url: "riskLimitCardNum/startLimitMemDay",
				  	data: "tid="+tid,
				 	dataType: "text",
				 	success:function(text){
				 		text=JSON.parse(text);
