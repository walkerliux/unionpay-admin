...
<body>
	<div style="margin: 5px; border:" id="continer">
		<div id="p" class="easyui-panel" title="商户单日累计限额查询"
		...
		var panelVertFloat = 150;
		 ...
		 $(function(){
			...
			$('#test').datagrid({
				...
				nowrap: false,
				striped: true,
				url:'riskLimitAmount/queryLimitMemNumsDay',
				remoteSort: false,
				columns:[
				[
...
//			$('#Notes').val('');
			...
			showRiskLevel();
			$('#theForm :input').val('');
			$("#theForm").attr("action","riskLimitAmount/saveLimitMemAmountDay");
			$('#w').window({
				title: '商户累计限额信息',
				top: panelVertFloat, 
		  		width: panelWidth,
	...
	function showLimitMem(tid){		
		$.ajax({
		   type: "POST",
		   url: "riskLimitAmount/queryLimitMumDaybytid",
		   data: "tid="+tid,
		   dataType:"json",
		   success: function(json){
		...
		$("#theForm").attr("action","riskLimitAmount/updateLimitMemMNumDay");
		$('#btn_submit').linkbutton('enable'); 
	}
	function deleteLimitMem(tid){
		$.messager.confirm('提示','您是否想要注销此日商户累计限额?',function(r){   
		    if (r){  
		    	$.ajax({
					type: "GET",
				  	url: "riskLimitAmount/deleteLimitMemAmountDay",
				  	data: "&tid="+tid,
				 	dataType: "text",
	...
	function startLimitMem(tid){
		$.messager.confirm('提示','您是否想要启用此商户日累计限额?',function(r){   
		    if (r){  
		    	$.ajax({
					type: "GET",
				  	url: "riskLimitAmount/startLimitMemDay",
				  	data: "tid="+tid,
				 	dataType: "text",
				 	success:function(text){
				 		text=JSON.parse(text);
