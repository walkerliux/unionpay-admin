...
<body>
	<div style="margin: 5px; border:" id="continer">
		<div id="p" class="easyui-panel" title="银行卡单日累计限额查询"
			...
			<div region="center" border="false"
				...
				<form id="theForm" method="post" action="">
					...
					<table width="100%" cellpadding="2" cellspacing="2"
						...
						<tr>
							<td align="right" width="15%" height="30px">卡类型</td>
							<td align="left" style="padding-left: 5px" width="25%">
							<select id="cardtype" class="easyui-validatebox" missingMessage="请选选择卡类型" required="true"
								name="cardtype">
		...
		var panelVertFloat = 150;
		 ...
		 $(function(){
			...
			$('#test').datagrid({
				...
				nowrap: false,
				striped: true,
				url:'riskLimitCardAmt/queryLimitMemNumsDay',
				remoteSort: false,
				columns:[
				[
...
//			$('#Notes').val('');
			...
			showCardtype();
			$('#theForm :input').val('');
			$("#theForm").attr("action","riskLimitCardAmt/saveLimitMemNumDay");
			$('#w').window({
				title: '银行卡累计限额信息',
				top: panelVertFloat, 
		  		width: panelWidth,
	...
	function showLimitMem(tid){		
		$.ajax({
		   type: "POST",
		   url: "riskLimitCardAmt/queryLimitMumDaybytid",
		   data: "tid="+tid,
		   dataType:"json",
		   success: function(json){
			 showriskid(json.riskname);
			$("#notes").val(json.notes);
			$("#limitAmount").val(json.limitAmount);
		...
		$("#theForm").attr("action","riskLimitCardAmt/updateLimitMemMNumDay");
		$('#btn_submit').linkbutton('enable'); 
	}
	function deleteLimitMem(tid){
		$.messager.confirm('提示','您是否想要注销此日银行卡累计限额?',function(r){   
		    if (r){  
		    	$.ajax({
					type: "GET",
				  	url: "riskLimitCardAmt/deleteLimitMemNumDay",
				  	data: "&tid="+tid,
				 	dataType: "json",
				 	success : function(json) {
						if (json.resultBool == true) {
	...
	function startLimitMem(tid){
		$.messager.confirm('提示','您是否想要启用此银行卡日累计限额?',function(r){   
		    if (r){  
		    	$.ajax({
					type: "GET",
				  	url: "riskLimitCardAmt/startLimitMemDay",
					data: "tid="+tid,
				 	dataType: "json",
				 	success : function(json) {
						if (json.resultBool == true) {
