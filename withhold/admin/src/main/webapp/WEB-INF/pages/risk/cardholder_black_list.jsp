	...
	<div style="margin: 5px; border:" id="continer">
		<div id="p" class="easyui-panel" title="持卡人黑名单查询"
			...
			<form id="searchForm" method="post">
				<table width="100%">
					<tr>
						<td align="right">持卡人身份证号</td>
						<td align="left" style="padding-left: 5px">
							<input id="idnum_s" maxlength="19" />
						</td>
		...
		<div style="margin-top: 5px">
			<table id="cardholderBlackList"></table>
		</div>
	</div>
	<div id="w" class="easyui-window" closed="true" title="My Window"
		...
		<div class="easyui-layout" fit="true">
			<div region="center" border="false"
				...
				<form id="theForm" method="post" action="">
					...
					<table width="100%" cellpadding="2" cellspacing="2" style="text-align: left">
						...
						<tr>
							<td class="update" width="15%">持卡人身份证号</td>
			...
			<div region="south" border="false"
				...
				<a class="easyui-linkbutton" iconCls="icon-ok" href="javascript:saveBlackIdnum()" id="btn_submit" onclick="">保存</a>
			</div>
		</div>
	</div>
</body>
...
<script>
		...
		$(function() {
			$('#cardholderBlackList').datagrid({
				...
				},
				{
					...
					formatter: function(value, rec) {
						if (rec.status == "00") {
							return '<a href="javascript:showBlackIdnum(' + value + ')" style="color:blue;margin-left:10px">修改</a>&nbsp;&nbsp;<a href="javascript:logoutCardholderBlackList(' + value + ')" style="color:blue;margin-left:10px">注销</a>';
						} else {
							return '<a href="javascript:startCardholderBlackList(' + value + ')" style="color:blue;margin-left:10px">启用</a>';
						}
					}
				}]],
				pagination: true,
			...
			var p = $('#cardholderBlackList').datagrid('getPager');
			$(p).pagination({
		...
		function search() {
			...
			$('#cardholderBlackList').datagrid('load', data);
		}
		...
		function showBlackIdnum(tid) {
			...
			$.ajax({
				type: "POST",
				url: "risk/queryCardholderBlackListByTid",
				data: "tid=" + tid,
				dataType: "json",
				success: function(json) {
		...
		function logoutCardholderBlackList(tid) {
			$.messager.confirm('提示', '您是否想要注销此持卡人黑名单?',
			function(r) {
				if (r) {
					$.ajax({
						type : "POST",
						url : "risk/logoutCardholderBlackList",
						dataType : "json",
						data : {
							"tid" : tid
						},
		...
		function startCardholderBlackList(tid) {
			$.messager.confirm('提示', '您是否想要启用此持卡人黑名单?',
			function(r) {
				if (r) {
					$.ajax({
						type : "POST",
						url : "risk/startCardholderBlackList",
						dataType : "json",
						data : {
							"tid" : tid
						},
