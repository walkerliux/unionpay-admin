...
<script>
		...
		$(function() {
			$('#cardWhiteList').datagrid({
				...
				},
				{
					...
					formatter: function(value, rec) {
						if (rec.status == "00") {
							return '<a href="javascript:showWhitePan(' + value + ')" style="color:blue;margin-left:10px">修改</a>&nbsp;&nbsp;<a href="javascript:logoutCardWhiteList(' + value + ')" style="color:blue;margin-left:10px">注销</a>';
						} else {
							return '<a href="javascript:startCardWhiteList(' + value + ')" style="color:blue;margin-left:10px">启用</a>';
						}
					}
				}]],
				pagination: true,
			...
			var p = $('#cardWhiteList').datagrid('getPager');
			$(p).pagination({
		...
		function search() {
			...
			$('#cardWhiteList').datagrid('load', data);
		}
		...
		function showWhitePan(tId) {
			...
			$('#theForm :input').val('');
			$("#theForm").attr("action", "risk/updateCardWhiteList");
			$('#btn_submit').linkbutton('enable');
			$.ajax({
				type: "POST",
				url: "risk/queryCardWhiteListByTid",
				data: "tId=" + tId,
				dataType: "json",
				success: function(json) {
					$("#pan").val(json.pan);
		...
		function logoutCardWhiteList(tId) {
			$.messager.confirm('提示', '您是否想要注销此银行卡白名单?',
			function(r) {
				if (r) {
					$.ajax({
						type : "POST",
						url : "risk/logoutCardWhiteList",
						dataType : "json",
						data : {
							"tId" : tId
						},
		...
		function startCardWhiteList(tId) {
			$.messager.confirm('提示', '您是否想要启用此银行卡白名单?',
			function(r) {
				if (r) {
					$.ajax({
						type : "POST",
						url : "risk/startCardWhiteList",
						dataType : "json",
						data : {
							"tId" : tId
						},
